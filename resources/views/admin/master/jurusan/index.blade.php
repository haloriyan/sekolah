...
<div class="p-10">
    ...
    <div class="flex items-center gap-4">
        ...
        <a href="{{ route('admin.master.jurusan.create') }}" class="bg-primary text-white p-3 px-6 rounded font-medium flex items-center gap-4">
                ...
                @foreach ($jurusans as $item)
                    <tr>
                        <td class="p-1">
                            @if ($item->image != null)
                                <img 
                                    src="{{ asset('storage/foto_jurusan/' . $item->image->filename) }}" 
                                    alt="{{ $item->title}}"
                        ...
                        <td class="p-1 flex items-center gap-2">
                            <a href="{{ route('admin.master.jurusan.edit', $item->id) }}" class="cursor-pointer text-white bg-primary p-2 px-4 text-sm mt-4">
...
<div class="fixed top-0 left-0 right-0 bottom-0 bg-white bg-opacity-75 flex items-center justify-center hidden" id="deleteModal">
    <form class="bg-white shadow-lg rounded-lg p-10 w-4/12 mobile:w-10/12" method="POST" action="{{ route('admin.master.jurusan.delete') }}">
        @csrf
        <input type="hidden" name="id" id="id">
        <h3 class="text-lg text-slate-700 font-medium">Hapus Jurusan</h3>
        <div class="text-sm text-slate-500 mt-2">Yakin ingin menghapus data jurusan ini?</div>
