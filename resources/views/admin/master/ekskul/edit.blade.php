...
@section('content')
<form class="flex gap-8 p-10" method="POST" enctype="multipart/form-data" action="{{ route('admin.master.ekskul.update', $ekskul->id) }}">
    @csrf
    <div class="flex flex-col w-7/12 gap-4 bg-white rounded-lg shadow-lg p-10 pb-10">
        ...
        <input type="text" name="title" class="w-full h-14 border px-6 outline-0" value="{{ $ekskul->title }}" required>

        <div>
            <div id="editor" class="h-96">{!! $ekskul->description !!}</div>
        </div>
        <textarea name="content" id="content_body" class="hidden">{{ $ekskul->description }}</textarea>
    </div>
            ...
            <div class="flex gap-4 flex-wrap" id="renderImages">
                @foreach ($ekskul->images as $img)
                    ...
                    <div class="w-24 aspect-square rounded-lg relative group flex items-center justify-center">
                        <img 
                            src="{{ asset('storage/foto_ekskul/' . $img->filename) }}" 
                            alt="{{ $img->filename}}"
